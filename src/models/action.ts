...
export const insertOrReplace = async (action: TrelloCardAction) => {
  ...
  return db.run(
    "INSERT OR REPLACE INTO `action` (id, member_id, name, type, date, card_id, board_id) VALUES (?, ?, ?, ?, ?, ?, ?)",
    action.id,
...
export const findByBoardId = async (boardId: string) => {
  ...
  return db.all(
    "SELECT id, member_id, name, type, datetime(date, 'localtime') as date, card_id, board_id FROM `action` WHERE board_id = ? ORDER BY date DESC",
    boardId
  );
};
